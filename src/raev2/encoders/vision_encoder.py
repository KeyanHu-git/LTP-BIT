...
class DINOv2Encoder(VisionEncoder):
    ...
    _LOCAL_ROOT = Path(__file__).resolve().parents[3] / "Weight" / "RAEv2" / "encoders" / "dinov2"
    ...
    def load_model(self):
        ...
        local_repo = os.path.join(local_root, "dinov2-main")
        suffix = "_registers_state_dict" if use_reg else ""
        local_ckpt = os.path.join(local_root, f"dinov2_vit_{base_config}14{suffix}.pth")
        if use_reg and not os.path.exists(local_ckpt):
            local_ckpt = os.path.join(local_root, f"dinov2_vit_{base_config}14_registers.pth")

        def _load_local():
            if not (os.path.isdir(local_repo) and os.path.exists(local_ckpt)):
                return None
...
class SkySenseHREncoder(VisionEncoder):
    """SkySense HR RGB backbone adapter for REPA target features.

    Config format: ``skysense-hr-stage2``. The stage-2 Swin feature map is
    flattened to patch tokens, giving 16x16 tokens for 256x256 inputs.
    """
    ...
    def load_model(self):
        if self.architecture != "hr" or self.model_config != "stage2":
            raise ValueError(
                "SkySense target encoder currently supports only "
                "'skysense-hr-stage2'."
            )
        from tools.data_curation.engine.metrics.coverage.skysense import (
            SkySenseHRConfig,
            load_skysense_hr_backbone,
        )

        cfg = SkySenseHRConfig(
            device=str(self.device),
            input_size=int(self.resolution),
            out_indices=(2,),
            feature_mode="last_pool",
        )
        self.model = load_skysense_hr_backbone(cfg)
        self.model.eval()
        self._embed_dim = 1408
        self.patch_size = 16

    def preprocess(self, x: torch.Tensor) -> torch.Tensor:
        mean = torch.tensor(self.RGB_MEAN, dtype=x.dtype, device=x.device).view(1, 3, 1, 1)
        std = torch.tensor(self.RGB_STD, dtype=x.dtype, device=x.device).view(1, 3, 1, 1)
    ...
    def forward_features(self, x: torch.Tensor) -> Dict[str, Optional[torch.Tensor]]:
        ...
        patch_tokens = feat.flatten(2).transpose(1, 2).contiguous().float()
        return {
            'x_norm_clstoken': None,
            'x_norm_patchtokens': patch_tokens,
        }
...
class PEEncoder(VisionEncoder):
    ...
    def load_model(self):
        from raev2.encoders.models import pe
