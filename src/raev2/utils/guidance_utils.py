...
from raev2.configs.stage2 import GuidanceConfig
...
def _use_cfg(guid_cfg):
    cfg = _get(guid_cfg, "cfg")
    return cfg is not None and float(_get(cfg, "scale", 1.0)) > 1.0


def _use_ig(guid_cfg):
    ig = _get(guid_cfg, "ig")
    return ig is not None and (
        float(_get(ig, "scale", 1.0)) > 1.0
        or (_get(ig, "unconditional_scale") is not None and float(_get(ig, "unconditional_scale")) != 1.0)
    )


def uses_cfg_guidance(guid_cfg) -> bool:
    return _use_cfg(guid_cfg)


def uses_internal_guidance(guid_cfg) -> bool:
    return _use_ig(guid_cfg)


def is_guidance_active(guid_cfg) -> bool:
    return uses_cfg_guidance(guid_cfg) or uses_internal_guidance(guid_cfg)


def duplicate_condition_kwargs(condition_kwargs: dict, batch_size: int) -> dict:
    """Duplicate per-sample tensor kwargs for [cond, uncond] guidance batches."""
    duplicated = {}
...
def slice_context_kwargs(condition_kwargs, batch_size):
    ...
    for key, value in condition_kwargs.items():
        if value is None:
            continue
        if isinstance(value, torch.Tensor) and value.ndim > 0 and value.shape[0] >= batch_size:
            half_kwargs[key] = value[:batch_size]
        else:
            half_kwargs[key] = value
    return half_kwargs


def _forward_with_base(model, x, t, **condition_kwargs):
    forward = getattr(model, "forward_with_base", None)
    if forward is not None:
        return forward(x, t, **condition_kwargs)
    return model(x, t, **condition_kwargs)
...
def forward_with_internalguidance(model, x, t, ig_scale, ig_interval=(0, 1), **condition_kwargs):
    ...
    full_out, base_out = _forward_with_base(model, half, t_half, **half_context_kwargs)
...
def forward_with_ig_and_cfg(
    ...
    full_out, base_out = _forward_with_base(model, x, t, **condition_kwargs)
...
def get_model_forward_fn(model, guid_cfg: GuidanceConfig):
    ...
    """
    use_ig = _use_ig(guid_cfg)
    use_cfg = _use_cfg(guid_cfg)

    if use_ig and use_cfg:
        # Combined IG + CFG
        model_fn = partial(forward_with_ig_and_cfg, model)
        sample_kwargs = dict(
            ig_scale=_get(_get(guid_cfg, "ig"), "scale"),
            cfg_scale=_get(_get(guid_cfg, "cfg"), "scale"),
            ig_interval=(_get(_get(guid_cfg, "ig"), "t_min"), _get(_get(guid_cfg, "ig"), "t_max")),
            cfg_interval=(_get(_get(guid_cfg, "cfg"), "t_min"), _get(_get(guid_cfg, "cfg"), "t_max")),
            uncond_ig_scale=_get(_get(guid_cfg, "ig"), "unconditional_scale"),
        )
    elif use_ig:
        # IG only
