...
class DDTEncoderBlock(nn.Module):
    ...
    def forward(self, x, rope, attn_mask=None, modulation=None, extra_kv=None):
        if modulation is None:
            x = x + self.attn(self.norm1(x), rope=rope, attn_mask=attn_mask, extra_kv=extra_kv)
            x = x + self.mlp(self.norm2(x))
            return x
        # Optional external FiLM (encoder blocks have no native adaLN). Zero
        # modulation equals the unmodulated path: (1+0)*f(x*(1+0)+0) == f(x).
        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = modulation.chunk(6, dim=-1)
        x = x + (1 + gate_msa) * self.attn(modulate(self.norm1(x), shift_msa, scale_msa), rope=rope, attn_mask=attn_mask, extra_kv=extra_kv)
        x = x + (1 + gate_mlp) * self.mlp(modulate(self.norm2(x), shift_mlp, scale_mlp))
        return x
...
class DDTDecoderBlock(DDTEncoderBlock):
    ...
    def forward(self, x, c, rope, attn_mask=None, modulation_delta=None, extra_kv=None):
        modulation = self.adaln_modulation(c)
        if modulation_delta is not None:
            modulation = modulation + modulation_delta.to(device=modulation.device, dtype=modulation.dtype)
        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = modulation.chunk(6, dim=-1)
        x = x + gate_msa * self.attn(modulate(self.norm1(x), shift_msa, scale_msa), rope=rope, attn_mask=attn_mask, extra_kv=extra_kv)
...
class DDTFinalLayer(nn.Module):
    ...
    def forward(self, x, c, modulation_delta=None):
        if len(c.shape) < len(x.shape):
            c = c.unsqueeze(1)
        modulation = self.adaln_modulation(c)
        if modulation_delta is not None:
            if len(modulation_delta.shape) < len(modulation.shape):
                modulation_delta = modulation_delta.unsqueeze(1)
            modulation = modulation + modulation_delta.to(device=modulation.device, dtype=modulation.dtype)
