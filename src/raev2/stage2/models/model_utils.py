...
def apply_rope_table(tokens, cos, sin):
    cos = cos.to(device=tokens.device, dtype=tokens.dtype)
    sin = sin.to(device=tokens.device, dtype=tokens.dtype)
...
class NormAttention(nn.Module):
    ...
    def forward(self, x, rope, attn_mask=None, extra_kv=None):
        B, N, _ = x.shape
        ...
        if extra_kv is not None:
            # (ek, ev, ebias): [B,H,Nk,hd]x2 + [B,1,1,Nk]|None; producer applies norm/rope.
            ek, ev, ebias = extra_kv
            k = torch.cat([k, ek.to(dtype=k.dtype)], dim=2)
            v = torch.cat([v, ev.to(dtype=v.dtype)], dim=2)
            if ebias is not None:
                if attn_mask is not None:
                    assert attn_mask.dim() == 4 and attn_mask.shape[-1] == N, \
                        f"extra_kv expects additive mask [B,1,1,{N}], got {tuple(attn_mask.shape)}"
                base = attn_mask if attn_mask is not None else q.new_zeros(B, 1, 1, N)
                attn_mask = torch.cat([base.expand(B, 1, 1, N), ebias.to(dtype=base.dtype)], dim=-1)
            elif attn_mask is not None:
                attn_mask = F.pad(attn_mask, (0, ek.shape[2]))
