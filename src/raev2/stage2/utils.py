...
from raev2.configs.stage2 import Stage2Config
from raev2.stage2.models.embedders import TextEncoder
from raev2.utils.dist_utils import main_process_first
...
def apply_cfg_dropout(model_conds, model_conds_null, cfg_dropout_prob=0.1):
    context = model_conds["context"]
    if cfg_dropout_prob <= 0:
        if isinstance(context, dict):
            first = next(v for v in context.values() if torch.is_tensor(v))
        else:
            first = context
        return model_conds, torch.zeros(first.shape[0], device=first.device, dtype=torch.bool)
    if isinstance(model_conds['context'], dict):
        from raev2.stage2 import nwm_cond
    ...
    for k, v in model_conds.items():
        if v is None or not torch.is_tensor(v) or k not in model_conds_null:
            dropped[k] = v
        else:
            dropped[k] = torch.where(mask.view(-1, *([1] * (v.ndim - 1))), model_conds_null[k], v)
    return dropped, mask
...
def setup_repa_target_encoder(config, rank, device, logger=None):
    """Build REPA target encoder when config.repa.use_repa is enabled."""
    repa = config.repa
    if not repa.use_repa:
        return None

    target_encoder = repa.target_encoder
    if not target_encoder:
        raise ValueError("repa.target_encoder is required when repa.use_repa=true.")

    resolution = int(repa.target_encoder_resolution or config.training.image_size)
    from raev2.encoders.vision_encoder import load_encoders

    with main_process_first(rank):
        encoder = load_encoders(str(target_encoder), device, resolution)[0]
    encoder.eval()
    encoder.model.requires_grad_(False)
    config.repa.z_dim = int(encoder.embed_dim)
    if logger is not None and rank == 0:
        logger.info(f"REPA target encoder: {target_encoder}, embed_dim={encoder.embed_dim}")
    return encoder
