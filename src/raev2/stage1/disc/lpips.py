...
class LPIPS(nn.Module):
    ...
    def extract_features_pair(self, input: torch.Tensor, target: torch.Tensor):
        input_scaled, target_scaled = self.scaling_layer(input), self.scaling_layer(target)
        return self.net(input_scaled), self.net(target_scaled)
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor, reduction: str = "mean") -> torch.Tensor:
        feats_input, feats_target = self.extract_features_pair(input, target)
        return self.forward_from_features(feats_input, feats_target, reduction=reduction)
